...
##        use += abs( temp - 50 ) - 15
    ...
    def choose_tariff( self, tariffs ):
        ...
        t_vec = [ (cost(t), t) for t in tariffs ]
        t_vec.sort()

        lim = [i for i in range(min( 3, len(t_vec) ))]
        choices = []
        for i in range( 50 ):
            choices.append( lim[0] )
        if len(lim) > 1:
            for i in range( 20 ):
                choices.append(lim[1])
        if len(lim) > 2:
            for i in range( 4 ):
                choices.append( lim[2] )

        idx = random.choice( choices )

        if self.tariff is not None and self.tariff.duration > 0:
            if t_vec[0][0] < cost(self.tariff):
                
                self.update_tariff( t_vec[idx][1] )
                return t_vec[idx][1]
            else:
                self.update_tariff( t_vec[idx][1] )
                return self.tariff
